...
class ACS_Color_Schemes {
	...
	function load_plugin_textdomain() {
		load_plugin_textdomain( 'admin-color-schemes', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
	}
	...
	function add_colors() {
		...
		wp_admin_css_color( 
			'vinyard', __( 'Vinyard', 'admin-color-schemes' ), 
			plugins_url( "colors/vineyard/colors$suffix.css", __FILE__ ),
			array( '#301D25', '#462b36', '#d3995d', '#eabe3f' ), 
			array( 'base' => '#f1f2f3', 'focus' => '#fff', 'current' => '#fff' )
		);

		wp_admin_css_color( 
			'primary', __( 'Primary', 'admin-color-schemes' ), 
			plugins_url( "colors/primary/colors$suffix.css", __FILE__ ),
			array( '#282b48', '#35395c', '#f38135', '#e7c03a' ),
			array( 'base' => '#f1f2f3', 'focus' => '#fff', 'current' => '#fff' )
		);

		wp_admin_css_color( 
			'80s-kid', __( '80\'s Kid', 'admin-color-schemes' ), 
			plugins_url( "colors/80s-kid/colors$suffix.css", __FILE__ ),
			array( '#0A3D80', '#0c4da1', '#ed5793', '#eb853b' ),
			array( 'base' => '#e4e5e7', 'focus' => '#fff', 'current' => '#fff' )
		);

		wp_admin_css_color( 
			'aubergine', __( 'Aubergine', 'admin-color-schemes' ), 
			plugins_url( "colors/aubergine/colors$suffix.css", __FILE__ ),
			array( '#4c4b5f', '#585a61', '#e87162', '#da9b49' ),
			array( 'base' => '#e4e4e7', 'focus' => '#fff', 'current' => '#fff' )
		);

		wp_admin_css_color( 
			'cruise', __( 'Cruise', 'admin-color-schemes' ), 
			plugins_url( "colors/cruise/colors$suffix.css", __FILE__ ),
			array( '#292B46', '#36395c', '#8bbc9f', '#d2ac1f' ),
			array( 'base' => '#f1f1f3', 'focus' => '#fff', 'current' => '#fff' )
		);

		wp_admin_css_color( 
			'flat', __( 'Flat', 'admin-color-schemes' ), 
			plugins_url( "colors/flat/colors$suffix.css", __FILE__ ),
			array( '#1F2C39', '#2c3e50', '#1abc9c', '#f39c12' ),
			array( 'base' => '#f1f2f3', 'focus' => '#fff', 'current' => '#fff' )
		);

		wp_admin_css_color( 
			'lawn', __( 'Lawn', 'admin-color-schemes' ), 
			plugins_url( "colors/lawn/colors$suffix.css", __FILE__ ),
			array( '#0F1515', '#1e2a29', '#5D824B', '#a7b145' ),
			array( 'base' => '#f1f3f3', 'focus' => '#fff', 'current' => '#fff' )
		);

		wp_admin_css_color( 
			'seashore', __( 'Seashore', 'admin-color-schemes' ), 
			plugins_url( "colors/seashore/colors$suffix.css", __FILE__ ),
